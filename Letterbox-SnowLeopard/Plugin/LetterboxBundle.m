...
- (IBAction)sendPluginFeedback:(id)sender
{
	...
	NSString *feedbackURLString = [NSString stringWithFormat: 
							 [bundle objectForInfoDictionaryKey:@"BundleFeedbackURL"],
							 [self version],
							 body];
	NSLog(@"Feedback URL: %@", feedbackURLString);
	NSURL *feedbackString = [NSURL URLWithString:[feedbackURLString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
	[[NSWorkspace sharedWorkspace] openURL:feedbackString];
}

- (IBAction)makePluginDonation:(id)sender
{
	[[NSWorkspace sharedWorkspace] openURL:
	 [NSURL URLWithString:[bundle objectForInfoDictionaryKey:@"BundleDonationURL"]]];
}
