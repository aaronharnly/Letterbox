...
id SUUnlocalizedInfoValueForKey(NSString *key)
{
	...
	id value = [[[SUUtilities bundleToUpdate]  infoDictionary] valueForKey:key];
	if (!value)
...
{
	return [[SUUtilities bundleToUpdate]  objectForInfoDictionaryKey:key];
}

NSString *SUHostAppName()
{
	if (SUInfoValueForKey(@"CFBundleName")) { return SUInfoValueForKey(@"CFBundleName"); }
	return [[[NSFileManager defaultManager] displayNameAtPath:[[SUUtilities bundleToUpdate]  bundlePath]] stringByDeletingPathExtension];
}

NSString *SUHostBundleExtension()
{
    if (SUInfoValueForKey(@"SUHostBundleExtension")) { return SUInfoValueForKey(@"SUHostBundleExtension"); }
    return  [[[NSFileManager defaultManager] displayNameAtPath:[[SUUtilities bundleToUpdate]  bundlePath]] pathExtension];
}
...
/**
 * Returns the bundle we want updated.
 * By default, this is the main application bundle.
 */
static NSBundle *bundleToUpdate = nil;
+(NSBundle *)bundleToUpdate
{
    if (bundleToUpdate == nil)
...
{
    if (bundleToUpdate != aBundle) {
	
    }
    bundleToUpdate = aBundle;
}

static NSImage *bundleIcon = nil;
+(NSImage *)bundleIcon
{
    if (bundleIcon == nil)
