...
//
// $Id: SUUpdater.m 56 2006-07-29 21:49:55Z atomicbird $
// $HeadURL: http://ironcoder.org/svn/SparklePlus/trunk/SUUpdater.m $
...
- init
{
	...
	if (!path) // slight hack to resolve issues with running with in configurations
	{
		...
		NSString *frameworkPath = [[[SUUtilities bundleToUpdate]  sharedFrameworksPath] stringByAppendingFormat:@"/Sparkle.framework", [current bundleIdentifier]];
...
- (void)applicationDidFinishLaunching:(NSNotification *)note
{
	...
	if ([self storedCheckInterval])
	{
		...
		NSDate *lastCheck = [[self defaults] objectForKey:SULastCheckTimeKey];
	...
	else
	{
		...
		NSNumber *shouldCheckAtStartup = [[self defaults] objectForKey:SUCheckAtStartupKey];
		NSNumber *shouldSendProfileInfo = [[self defaults] objectForKey:SUSendProfileInfoKey];
		...
		if (!(shouldCheckAtStartup && shouldSendProfileInfo)) {
			...
			[[self defaults] setObject:[NSNumber numberWithBool:YES] forKey:SUSendProfileInfoKey];
			...
			if (![myBundle loadNibFile:@"SUProfileInfo" externalNameTable:[NSDictionary dictionaryWithObject:self forKey:@"NSOwner"] withZone:nil]) {
				...
				NSString *frameworkPath = [[[SUUtilities bundleToUpdate]  privateFrameworksPath] stringByAppendingFormat:@"/Sparkle.framework"];
				NSBundle *framework = [NSBundle bundleWithPath:frameworkPath];
			...
			if (profileMoreInfoView != nil) {
				...
				[[self defaults] setObject:shouldCheckAtStartup forKey:SUCheckAtStartupKey];
				...
				[[self defaults] setObject:shouldSendProfileInfo forKey:SUSendProfileInfoKey];
...
{
	if ([delegate respondsToSelector:@selector(defaults)])
...
- (NSImage *)bundleIcon
{
	if ([delegate respondsToSelector:@selector(bundleIcon)])
		return [delegate bundleIcon];
	else
		return [NSApp applicationIconImage];
}
// Get the defaultsController from the delegate.
// If we don't manage that, then we're screwed.
// I suppose we could fall back to the standard controller, but then we're back in the main
//  application's preferences, instead of our own...
- (NSUserDefaultsController *)defaultsController
{
	if ([delegate respondsToSelector:@selector(defaultsController)])
		return [delegate defaultsController];
	else
		return [NSUserDefaultsController sharedUserDefaultsController];
}

- (NSView *)profileMoreInfoView
{
	if (profileMoreInfoView == nil) {
			NSString *frameworkPath = [[[SUUtilities bundleToUpdate]  privateFrameworksPath] stringByAppendingFormat:@"/Sparkle.framework"];
			NSBundle *framework = [NSBundle bundleWithPath:frameworkPath];
...
- (void)profileInfoSheetDidEnd:(NSWindow *)sheet returnCode:(int)returnCode  contextInfo:(void  *)contextInfo
{
	...
	if (returnCode == 1) {
		[[self defaults] setObject:[NSNumber numberWithBool:YES] forKey:SUCheckAtStartupKey];
	} else {
		[[self defaults] setObject:[NSNumber numberWithBool:NO] forKey:SUCheckAtStartupKey];
	}
	NSNumber *shouldCheckAtStartup = [[self defaults] objectForKey:SUCheckAtStartupKey];
	if ([shouldCheckAtStartup boolValue])
		[self checkForUpdatesInBackground];
...
- (NSMutableArray *)systemProfileInformationArray
{
	...
	NSArray *languages = [standardDefaults objectForKey:@"AppleLanguages"];
	...
	// Bundle sending the request
	NSString *bundleName = SUInfoValueForKey(@"CFBundleName");
	if (bundleName)
		[profileArray addObject:[NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:@"bundleName",@"Plugin Name", bundleName, bundleName,nil] forKeys:profileDictKeys]];
	NSString *bundleVersion = SUInfoValueForKey(@"CFBundleVersion");
	if (bundleVersion)
		[profileArray addObject:[NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:@"bundleVersion",@"Plugin Version", bundleVersion, bundleVersion,nil] forKeys:profileDictKeys]];
...
{	
	...
	NSString *appcastString = [[self defaults] objectForKey:SUFeedURLKey];
	if (!appcastString)
	...
	if ([delegate respondsToSelector:@selector(updaterShouldSendProfileInfo)]) {
		...
		if ([[[self defaults] objectForKey:SUSendProfileInfoKey] boolValue]) {
			appcastString = [self appendProfileInfoToURLString:appcastString];
		}
	}
	SUAppcast *appcast = [[SUAppcast alloc] init];
	[appcast setDelegate:self];
	NSLog(@"Checking appcast at URL: %@", appcastString);
...
- (BOOL)automaticallyUpdates
{
	...
	if (![[self defaults] objectForKey:SUAutomaticallyUpdateKey]) { return NO; } // defaults to NO
	return [[[self defaults] objectForKey:SUAutomaticallyUpdateKey] boolValue];
}
...
- (NSTimeInterval)storedCheckInterval
{
	...
	if ([[self defaults] objectForKey:SUScheduledCheckIntervalKey])
	{
		long interval = [[[self defaults] objectForKey:SUScheduledCheckIntervalKey] longValue];
		if (interval > 0)
			return interval;
	}
...
- (void)remindMeLater
{
	...
	[[self defaults] setObject:nil forKey:SUSkippedVersionKey];
...
- (void)updateAlert:(SUUpdateAlert *)alert finishedWithChoice:(SUUpdateAlertChoice)choice
{
	...
	switch (choice)
	{
		...
		case SUSkipThisVersionChoice:
			updateInProgress = NO;
			[[self defaults] setObject:[updateItem fileVersion] forKey:SUSkippedVersionKey];
			break;
	}			
}
...
- (void)appcastDidFinishLoading:(SUAppcast *)ac
{
	@try
	{
		...
		[[self defaults] setObject:[NSDate date] forKey:SULastCheckTimeKey];
		...
		if (!verbose && [[[self defaults] objectForKey:SUSkippedVersionKey] isEqualToString:[updateItem fileVersion]]) { updateInProgress = NO; return; }
	...
	@catch (NSException *e)
	{
		NSLog(@"Retrieving update information failed, because: %@",[e reason]);
		updateInProgress = NO;
		if (verbose)
			[self showUpdateErrorAlertWithInfo:SULocalizedString(
				[NSString stringWithFormat:@"An error occurred in retrieving update information. Please try again later.\n\nDetails:\n%@", [e reason] ]
				, nil)];
	}
}
...
- (void)unarchiverDidFinish:(SUUnarchiver *)ua
{
	...
	{
		[statusController beginActionWithTitle:SULocalizedString(@"Ready to install!", nil) maxProgressValue:1 statusText:nil authenticationIndicator:[ua authenticated]];
...
- (void)extractUpdate
{
	...
	@try 
	{
		...
		BOOL needsAuthentication = [SUInfoValueForKey(SUExpectsDSASignatureKey) boolValue];
		...
		SUUnarchiver *unarchiver = [[SUUnarchiver alloc] initAuthenticated:needsAuthentication];
		[unarchiver setDelegate:self];
		[unarchiver unarchivePath:downloadPath]; // asynchronous extraction!
	}
	@catch(NSException *e) {
		NSLog([e reason]);
		[self showUpdateErrorAlertWithInfo:
			[NSString stringWithFormat:
				SULocalizedString(@"An error occurred while extracting the archive. Please try again later.\n\nDetails: %@", nil),
				[e reason]]];
		[self abandonUpdate];
	}	
}
...
- (IBAction)installAndRestart:sender
{
	NSString *currentAppPath = [[SUUtilities bundleToUpdate]  bundlePath];
	...
	@try 
	{
		...
		NSString *file, *appName = [SUUnlocalizedInfoValueForKey(@"CFBundleName") stringByAppendingPathExtension:SUHostBundleExtension()];
		...
		{
			[NSException raise:@"SUInstallException" format:@"The update archive didn't contain an application with the proper name: %@. Remember, the updated app's file name must be identical to {CFBundleName}.app", [SUInfoValueForKey(@"CFBundleName") stringByAppendingPathExtension:SUHostBundleExtension()]];
		}
	}
	@catch(NSException *e) 
	{
